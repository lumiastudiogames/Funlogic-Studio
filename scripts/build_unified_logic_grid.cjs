...
// Write to all 20 game directories
const gamesDir = path.resolve(__dirname, '../src/games');
let count = 0;

for (let i = 1; i <= 20; i++) {
  const num = String(i).padStart(2, '0');
  const d = fs.readdirSync(gamesDir).find(x => x.startsWith(num + '-'));
  if (d) {
    const enginePath = path.join(gamesDir, d, 'engine.js');
    fs.writeFileSync(enginePath, UNIFIED_ENGINE_CODE, 'utf8');
    count++;
    console.log(`Updated [${count}/20]: ${d}/engine.js`);
  }
}

console.log(`Successfully updated unified LogicGridEngine across all ${count} numbered games!`);
