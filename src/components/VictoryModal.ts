...
export function showVictoryModal(options: {
  ...
  modalOverlay.className = 'fixed inset-0 z-[100] bg-black/70 backdrop-blur-xs flex items-center justify-center p-4 animate-fade-in';
  ...
  modalOverlay.innerHTML = `
    <div class="bg-white rounded-3xl p-6 sm:p-8 max-w-sm w-full text-center shadow-2xl border border-black/5 relative transform transition-all scale-100">
      
      <!-- Trophy Mini-Sticker -->
      <div class="w-20 h-20 p-2 mx-auto mb-4 filter drop-shadow-md animate-bounce">
        ${STICKERS.trophy}
      </div>

      <h2 class="font-extrabold text-2xl text-gray-900 mb-1">
        Puzzle Solved!
      </h2>
      ...
      <div class="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 mb-4 grid grid-cols-${options.streak ? '2' : '1'} gap-3">
        <div>
          <div class="text-[10px] font-extrabold text-emerald-700 uppercase tracking-widest">Time Taken</div>
      ...
      <!-- Quick Real Rating -->
      <div id="rating-box" class="bg-slate-50 border border-slate-200 rounded-2xl p-3 mb-5">
        ...
        <div id="rating-feedback" class="text-[10px] text-emerald-700 font-bold mt-1 hidden">Thanks for rating! ⭐</div>
      </div>

      <!-- Actions -->
      <div class="flex flex-col gap-2.5">
        <button id="btn-share" class="w-full h-12 bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white font-bold text-sm rounded-2xl shadow-sm transition flex items-center justify-center gap-2">
          <span>Share Result & Streak</span>
        ...
        <a href="#home" id="btn-home-victory" class="w-full h-12 bg-gray-100 hover:bg-gray-200 active:scale-95 text-gray-800 font-bold text-sm rounded-2xl transition flex items-center justify-center text-decoration-none">
          Back to FunLogic Home
        </a>
      </div>

      <!-- Close Button -->
      <button id="btn-close-victory" class="absolute top-4 right-4 text-gray-400 hover:text-gray-700 text-xl font-bold p-2 leading-none">
        ✕
      </button>
    </div>
  `;
  ...
  const shareText = `You solved ${options.title} in ${timeFormatted}! ${options.streak ? `🔥 ${options.streak} day streak!` : ''} Play on LoomLogic: ${window.location.origin}`;
  ...
  const closeModal = () => {
    modalOverlay.remove();
    anchorAd.show(); // Restore normal anchor ad behavior
    options.onClose();
  };
