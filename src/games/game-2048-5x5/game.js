/**
 * 2048 5x5 Expert - Smooth Sliding & Star Particle Merge Engine
 */
...
class Game2048_5x5 {
  ...
  spawnStarParticles(r, c, val) {
    ...
    const cellH = (containerRect.height - (this.size - 1) * 8) / this.size;
    const x = c * (cellW + 8) + cellW / 2;
    const y = r * (cellH + 8) + cellH / 2;
  ...
  move(direction) {
    ...
    if (moved) {
      ...
      if (this.score > this.bestScore) {
        ...
        this.bestScoreEl.textContent = this.bestScore;
        localStorage.setItem('2048_5x5_best', this.bestScore.toString());
      }
