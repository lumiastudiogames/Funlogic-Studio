/**
 * Daily Sudoku Challenge - Pure Vanilla JS
 */

class SoundEngine {
  constructor() {
    this.ctx = null;
    this.muted = localStorage.getItem('sudoku_daily_muted') === 'true';
  }
  init() {
    if (!this.ctx) {
  ...
  toggle() {
    ...
    localStorage.setItem('sudoku_daily_muted', this.muted);
    return this.muted;
  }
  play(type) {
...
class DailySudokuGenerator {
  static solve(board, rng) {
    const empty = this.findEmpty(board);
    if (!empty) return true;
    const [row, col] = empty;
    const digits = [1, 2, 3, 4, 5, 6, 7, 8, 9].sort(() => rng.next() - 0.5);

    for (const num of digits) {
      if (this.isValid(board, row, col, num)) {
        board[row][col] = num;
        if (this.solve(board, rng)) return true;
        board[row][col] = 0;
      }
    }
    return false;
  ...
  static generateForDate(dateStr) {
    const rng = new SeededRandom(dateStr);
    const board = Array.from({ length: 9 }, () => Array(9).fill(0));
    this.solve(board, rng);
    ...
    // 34 givens for Daily Sudoku
    const removals = 81 - 34;
...
class DailySudokuGame {
  constructor() {
    this.sound = new SoundEngine();
    this.currentDate = new Date();
    this.streak = parseInt(localStorage.getItem('sudoku_daily_streak') || '0', 10);
    this.lastWinDate = localStorage.getItem('sudoku_daily_last_win') || '';
    this.puzzle = [];
    this.solution = [];
    this.userGrid = [];
    ...
    this.cacheDOM();
    this.bindEvents();
    this.loadDatePuzzle(this.formatDate(this.currentDate));
  }
  ...
  cacheDOM() {
    this.gridEl = document.getElementById('sudoku-daily-grid');
  ...
  loadDatePuzzle(dateStr) {
    this.dateDisplayEl.textContent = this.formatDisplayDate(this.currentDate);
    const gen = DailySudokuGenerator.generateForDate(dateStr);
  ...
  checkWin() {
    ...
    const playedDateStr = this.formatDate(this.currentDate);

    if (playedDateStr === todayStr && this.lastWinDate !== todayStr) {
      const yesterday = new Date();
      yesterday.setDate(yesterday.getDate() - 1);
      const yesterdayStr = this.formatDate(yesterday);

      if (this.lastWinDate === yesterdayStr) {
        this.streak += 1;
      } else {
        this.streak = 1;
      }
      this.lastWinDate = todayStr;
      localStorage.setItem('sudoku_daily_streak', this.streak);
      localStorage.setItem('sudoku_daily_last_win', todayStr);
      this.streakValEl.textContent = this.streak;
    }
    ...
    this.winStreakEl.textContent = `${this.streak} Days`;
